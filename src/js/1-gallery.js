...
document.addEventListener('keydown', (event) => {
    if (event.key === '+' || event.key === '-') {
        event.preventDefault(); // Щоб уникнути стандартної поведінки
        const deltaY = event.key === '+' ? -100 : 100; // Імітація прокрутки вверх або вниз

        // Створюємо подію wheel
        ...
        // Викликаємо подію на поточному активному елементі
...
/*function navigationGallery(event) {
   console.log("🚀 ~ navigationGallery ~ navigationGallery:", navigationGallery)
        ...
        if ((event.key === 'A' || event.key === 'a' || event.key === '4' || event.key === "numpad4")) {
            activeIndex = (activeIndex > 0) ? activeIndex - 1 : galleryItems.length - 1;
            console.log("🚀 ~ navigationGallery ~ activeIndex:", activeIndex)
            updateActiveImg();
        }
        if ((event.key === 'D' || event.key === 'd' || event.key === '6' || event.key === "numpad6")) {
            activeIndex = (activeIndex < galleryItems.length - 1) ? galleryItems.length + 1 : 0;
            console.log("🚀 ~ navigationGallery ~ activeIndex:", activeIndex)
            updateActiveImg();
        }
